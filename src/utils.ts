...
export type FstArg<T> = T extends (arg1: infer U, ...args: any[]) => any
  ? U
  : any;
export type SndArg<T> = T extends (
  arg1: any,
  arg2: infer U,
  ...args: any[]
) => any
...
export function getAgent(endpoint: UrlWithStringQuery): Agent {
  ...
  if (env) {
    ...
    if (noProxy === "*") {
      ...
      const port =
        endpoint.port || endpoint.protocol.startsWith("https") ? "443" : "80";
  ...
  if (proxy) {
    ...
    let parts = auth.length
      ? proxy.slice(auth.length + 1).split(":")
      : proxy.split(":");
    if (parts.length > 1) {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    const req = https.request(options, (res: IncomingMessage) => {
      if (res.statusCode != 200) {
        reject(
          new Error(
            `Invalid response from ${JSON.stringify(url)}: ${res.statusCode}`
          )
        );
        return;
      }
